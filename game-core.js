...
(function (root) {
  ...
  function newGame(opts) {
    ...
    var st = {
      ...
      distractions: [],
      block: 0,
      target: null,            // currently selected Trial id (free to switch anytime)
      resolveOpen: false,      // progress window open (just finished a Pomodoro)
      recommended: null,       // eisenhower suggestion
      status: "playing",
      completed: [],           // {text, difficulty, outcome} as Trials leave the board
      stats: { focusGenerated: 0, focusSpent: 0, distractionsHandled: 0, distractionsExploded: 0 },
      log: [],
      _rng: rng,
  ...
  function removeTask(st, id, reason) {
    ...
    var outcome = /SUPERATA/.test(reason || "") ? "superata"
                : /affidata/i.test(reason || "") ? "affidata" : "rimossa";
    st.completed.push({ text: t.text, difficulty: t.maxToughness, outcome: outcome });
  ...
  function tapLand(st, landId) {
    ...
    st.stats.focusGenerated += def.produce;
  ...
  function completeBlock(st, targetId) {
    ...
    if (tgt && tgt.enchanted && st.distractions.length) {
      ...
      st.stats.distractionsHandled += 1;
  ...
  function advanceDistractions(st) {
    ...
    st.distractions.forEach(function (d) {
      ...
      st.stats.distractionsExploded += 1;
      // trigger effect
  ...
  function dismissDistraction(st, id) {
    ...
    st.focus -= 1; st.stats.focusSpent += 1; st.stats.distractionsHandled += 1;
  ...
  function playCard(st, cardId, targetId) {
    ...
    if (c.kind === "damage") {
      ...
      st.focus -= c.cost; st.stats.focusSpent += c.cost;
      tgt.toughness -= c.dmg;
    ...
    if (c.kind === "protect") {
      if (st.focus < c.cost) return { ok: false, reason: "Focus insufficiente" };
      st.focus -= c.cost; st.stats.focusSpent += c.cost; tgt.enchanted = true;
      st.stats.timeBlocksUsed = (st.stats.timeBlocksUsed || 0) + 1;
      log(st, "Time Block su " + tgt.text + ": protetto.");
      return { ok: true };
    }
    if (c.kind === "remove") {
      if (st.focus < c.cost) return { ok: false, reason: "Focus insufficiente" };
      st.focus -= c.cost; st.stats.focusSpent += c.cost; removeTask(st, tgt.id, "Prova affidata ad altri");
      return { ok: true };
    }
    if (c.kind === "sort") {
      if (st.focus < c.cost) return { ok: false, reason: "Focus insufficiente" };
  ...
  // Build a structured summary of the current day, for history/progression.
  // focusMinutes is supplied by the UI (sum of completed Pomodoro durations).
  function daySummary(st, opts) {
    opts = opts || {};
    var superate = st.completed.filter(function (c) { return c.outcome === "superata"; }).length;
    return {
      result: st.status === "won" ? "vinta" : st.status === "lost" ? "persa" : "chiusa",
      trials: st.completed.slice(),
      trialsLeft: st.tasks.map(function (t) {
        return { text: t.text, difficulty: t.maxToughness, outcome: "non_completata" };
      }),
      trialsTotal: st.completed.length + st.tasks.length,
      trialsDone: superate,
      trialsDelegated: st.completed.filter(function (c) { return c.outcome === "affidata"; }).length,
      pomodoros: st.block,
      focusMinutes: opts.focusMinutes || 0,
      focusGenerated: st.stats.focusGenerated,
      focusSpent: st.stats.focusSpent,
      distractionsHandled: st.stats.distractionsHandled,
      distractionsExploded: st.stats.distractionsExploded,
      timeBlocksUsed: st.stats.timeBlocksUsed || 0,
      maxDifficultyCleared: st.completed.reduce(function (m, c) {
        return c.outcome === "superata" ? Math.max(m, c.difficulty) : m;
      }, 0),
      diff3Cleared: st.completed.filter(function (c) { return c.outcome === "superata" && c.difficulty >= 3; }).length
    };
  }
